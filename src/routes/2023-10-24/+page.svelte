...
<Post>
  <article slot="post_content">
    <p>
      ...
      作為我的主要筆記軟體，使用了一陣子後又發現有一些缺點讓我有點無法忍受導致我又換回了 notion ，而在前一陣子遇上了
      heptabase 目前知識管理及記事的流程就大致定下來了。
    </p>
    <blockquote>
    ...
    <p>
      一切的起點是我看了一本叫做「卡片盒筆記術」的書，原本我就對於生產力感到焦慮所以那時就覺得這個筆記方法也許對我很有幫助吧。下定決心要開始實踐時就陷入一個寫筆記前的萬年難題「到底該選哪種筆記軟體？」，我自己認為「卡片盒筆記」最為重要的事應該是「連結」這件事情，所以那時就先放棄了
      notion 。
    </p>
    <blockquote>
    ...
    <p>
      ...
      database 或者 page 下，他該是一份新的 page 還是記在某個已存在的 page 就好呢？
    </p>
    <p>
      總而言之這種以 page
      作為形式的思考雖然結構嚴謹，但有時候就是真的不知道結構該長怎麼樣所以在寫筆記前總會想很多。但記筆記有時候就是只是為了記錄一閃而過的想法，但每次想筆記的結構都比寫筆記的時間還長。當然
      notion 就算事後要改變一個 page 屬於哪個 database
      也不是辦不到，但操作起來就是有一點麻煩，所以才會變成一開始就希望放對地方。
    </p>
    <p>
      而 obsidian 雖然還是 page 為單位而且也有 folder 的概念，但因為沒有 database
      所以基本上我可以隨便寫，不用想到底該放在哪，至於 folder 只是整理 page
      的存放位置，但就算放在一起也不會有什麼真正的「連結」出現。
    </p>
    <p>
    ...
    <p>總之 obsidian 對我來說是一個相當不錯的筆記軟體，但還是有些點讓我無法忍受以致於我棄用了。</p>
    <h1 id="wei4-shen2-me-huan4-hui2-notion"><a href="#wei4-shen2-me-huan4-hui2-notion">為什麼換回 notion</a></h1>
    <p>
      約莫用了半年大致上體驗還是相當不錯，這時我算是使用 notion + obsidian 的形式在使用， notion
      就當作收藏文章使用，畢竟它的 chrome 插件相當好用。然後 obsidian 就做扣除收集文章以外的事情。
    </p>
    <p>
      但前面有說到我有時候會兩台筆電交叉使用的，其實這點也跟工作型態改變有那麼一點點關係
      xD，就是我們公司開始要大家回辦公室了，所以以前其實就算下班在家如果要寫筆記或文章也會使用公司的電腦，但變成大部分時間在辦公室後我算是蠻懶的背電腦回家的人所以如果下班在家要寫筆記就變成必須使用我的舊電腦。
    </p>
    <p>
      前面有提到 obsidian git 插件這件事情，因為 obsidian 是只有
      <strong>「本地儲存」</strong>
      的也就是說如果我沒有特別同步的話我在公司電腦寫的筆記就無法再我自己的電腦上繼續撰寫，反之依然。
    </p>
    <p>
    ...
    <p>
      但是就是這個但是，因為是兩台電腦自動同步（其實就是自動 commit + push ）所以很多時候會遇到 conflict
      ，大部分原因都是因為我寫完後如果還沒到自動的同步的時間就關掉的話，這就是一個 unstage change
      ，所以我用另外一台電腦當然不會有拿到那份筆記，那如果我同時在另外一台電腦寫筆記時，我再次回到第一台電腦時就會遇到
    ...
    <p>
      這點也是我最不舒服的一點，除非我要每次關機前記得要手動同步一次不然隔天就是會遇到 conflict 。久而久之我就放棄使用
      obsidian 了轉而回去使用 notion 了。
    </p>
    <blockquote>
    ...
    <p>
      大多數人使用筆記軟體是要管理自己的知識、工作甚至是生活。但我相信部分的人（至少我是）都是一股腦把全部東西「放起來」而沒有
      「整理」過。
    </p>
    <p>
      回到 notion 第一件事情就是檢討一下自己為什麼之前用不好 notion
    ...
    <p>
      簡單介紹一下 heptabase
      ，最主要的點是將各種筆記或者該說是知識、想法變成「卡片」然後在各個「白板」中組合出一個新的結構。
    </p>
    <blockquote>
      <p>
    ...
    <p>
      這跟我使用 notion 時的想法完全不一樣，因為通常我是先想「我該寫在哪裡」之後在開始寫筆記。而 heptabase
      可以讓我「先寫就對了」之後再看怎麼整理。
    </p>
    <p>
    ...
    <p>
      所以目前我大部分有關「寫」這件事情都會在 heptabase 上處理，除了寫 blog 以外，畢竟寫 blog
      文章還算是一個比較有規則的寫作，只是我在寫 blog 的當下另外一個螢幕就會是看 hetpabase
      關於我這次想寫的主題的相關知識，而不用像以前一樣要在 notion 大海撈針。
    </p>
    <p>其餘不管是收集網路文章、旅遊景點、購物清單等等這種適合「用表格呈現」的行為我就會在 notion 裡處理。</p>
    ...
    <p>
      ...
      (其實我不確定是不是這個職稱，但總之就是負責客戶關係的成員)，前陣子開使提供預約諮詢，可以跟他討論有關 hetpabase
      各種事情。後來也有與它進行討論一些關於如何統整知識以及 heptabase 哪些功能可能可以達成我的需求。
    </p>
    <p>
      經過這次諮詢我覺得對於使用 heptabase
      十分有幫助，畢竟這個軟體其實有那麼一難上手，但我覺得困難的點是因為跟過去習慣的概念不太一樣，但我試著改用他們建議的使用方法後體驗好非常多。
    </p>
    <p>在「回到Axton」這個 yt 頻道聽過這麼一段話原文有點忘了，大意大概是：</p>
    <p><strong>「最享受的浪費時間方式，就是研究各種提升效率的方法」</strong></p>
    <p>只能說，確實，其實現在我想做的事情用 notion 也許可以符合，但就是沒有那麼舒服而已。</p>
  </article>
</Post>
