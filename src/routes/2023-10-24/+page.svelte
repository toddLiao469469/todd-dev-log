...
<Post>
  <article slot="post_content">
    <p>
      大概在去年年底，我開始使用了 obsidian
      作為我的主要筆記軟體，使用了一陣子後又覺得有一些我無法忍受的缺點導致我又換回了 notion ，而在前一陣子遇上了
      heptabase 也讓我目前知識管理及記事的流程就大致定下來了。
    </p>
    <blockquote>
    ...
    <p>
      這部分我原本在 notion 的做法是把不一樣的紀錄用各種 database 分開存放甚至會把收集文章的database
      拆成前端、後端、flutter 之類的主題。
    </p>
    <p>
      這代表我是先設立一堆架構之後再根據文章種類放進對應的
      database，這樣子的好處是部分滿足我的強迫症，但這也帶來最困擾我的兩個痛點「有時候我根本不知道該把這個筆記放到哪裡」以及「下次我該怎麼利用這份筆記」
    </p>
    <p>
      即使結構清晰是好事，可是因為 notion 的特性我蠻難一次抓取相關筆記所以我大概只能從各個可能有關 database
      找找看或者全域搜尋，只是沒有一個在 page 這個概念之上的值（其實就是類似 heptabase 或 obsidian 中的標籤）讓我去
      filter 。
    </p>
    <p>
    ...
    <p>之後大概爬了一些文後就選擇了用 obsidian ，有幾個優點我現在依然覺得蠻棒的</p>
    <ol>
      <li>啟動速度以及流暢度比起 notion 快很多很多</li>
      <li>寫筆記的阻力比起 notion 小很多</li>
      <li>雙向連結</li>
      <li>高度客製化及插件生態系</li>
    </ol>
    <p>
      ...
      版的體驗比較好後就都轉用 web 版了，但也就好那麼一點。那 obsidian 基本上在我兩台筆電上運作的都算順暢。
    </p>
    <p>
      可能是基於個性問題我認為用 notion 寫一份筆記是相當吃力的事情，因為我必須先想一下這份筆記應該放在哪個 database 或者
      page 下，他該是一份新的 page 還是記在某個已存在的 page 就好呢？這點會讓我覺得寫筆記的阻力相當的大。
    </p>
    <p>
      這種以 page
      作為形式的思考雖然結構嚴謹，但有時候就是真的不知道結構該長怎麼樣變成在寫筆記前會想太多，但筆記有時候就是只是為了記錄一閃而過的想法，所以就會變成想結構的時間比寫筆記還長，久而久之就會令人十分煩躁。當然就算事後要在
      notion 改變一個 page 是屬於哪個 database 也不是辦不到，但操作起來就是有一點麻煩，所以就會想一開始就放對地方。
    </p>
    <p>
      而 obsidian 雖然還是 page 為單位而且也有 folder 的概念，但因為沒有 database
      所以基本上我可以隨便寫，不用想到底該放在哪，至於 folder 只是整理 page
      的存放位置，即使放在一起也不會有什麼真正的「連結」出現。
    </p>
    <p>
      也因為在 obsidian 沒有什麼階層概念所以我加上一個標籤不會像 notion 那樣他是屬於在某個 database 的 column
      ，而算是一整個 vault （obsidian 的儲存單位，可以想像成 root 資料夾就是了）都是範圍的 tag，我只需要打上 tag <code
    ...
    <p>
      以我之前做 rust 筆記為例，首先我先寫了一個「Rust 所有權」的筆記，後來有一天我在寫 「struct method
    ...
    <p>
      前面有說到我有時候會兩台筆電交叉使用的，其實這點也跟工作型態改變有那麼一點點關係
    ...
    <p>
      ...
      也不會是什麼困難的事情，再加上有插件其實我也很少手動下指令。
    </p>
    <p>
      但是就是這個但是，因為是兩台電腦自動同步（其實就是自動 commit + push ）所以很常會遇到 conflict
      ，大部分原因都是因為我寫完後如果還沒到自動的同步的時間就關掉的話，就是會留下一些 unstage change
      ，所以我用另外一台電腦當然不會拿到那份變更過筆記，那如果我同時在另外一台電腦寫筆記時，我再次回到第一台電腦時就會遇到
      conflict 了。以及 obsidian 有一個本地檔案是紀錄目前 obsidian 的 config ，也就是目前這個 vault
      有哪些筆記、視窗設定是什麼等等，所以如果我在另外一台電腦調了一下視窗大小也會導致這個檔案的變動，導致這個檔案也相當常遇到
      conflict 。
    </p>
    <p>
      這點就是我最不舒服的一點，除非我要每次關機前記得要手動同步一次不然隔天就是會遇到 conflict
      ，更別說我有時候也會用手機或者平板記事在這些行動裝置上同步更是一場災難，久而久之我就放棄使用 obsidian
      了轉而回去使用 notion 了。
    </p>
    ...
    <p>
      大多數人使用筆記軟體是要管理自己的知識、工作甚至是生活，但我相信部分的人（至少我是）都是一股腦把全部東西「放起來」而沒有「整理」過，所以回到
      notion 後的第一件事情就是檢討一下自己為什麼之前用不好 notion
      ，簡單來說還是這兩個問題「有時候我根本不知道該把這個筆記放到哪裡」、「下次我該怎麼利用這份筆記」。
    </p>
    <h2 id="bi3-ji4-de-ru4-kou3"><a href="#bi3-ji4-de-ru4-kou3">筆記的入口</a></h2>
    <p>
    ...
    <h3 id="gong1-zuo4-bi3-ji4"><a href="#gong1-zuo4-bi3-ji4">工作筆記</a></h3>
    <p>
      工作筆記比較像通常是我在開發某個功能或產品時會用到，通常筆記規模會是一個 feature
      那我開發時遇到什麼問題或者開會上有提到哪些事情都會寫在同一頁。
    </p>
    <h3 id="wen2-zhang1-zhuan4-xie3-jin4-du4"><a href="#wen2-zhang1-zhuan4-xie3-jin4-du4">文章撰寫進度</a></h3>
    <p>這邊就是完全為了部落格而設計的區塊了，簡單來說就是讓我總覽一下現在有哪些文章寫到什麼進度。</p>
    <p>
      暫時解決我下次要找一個筆記或者要寫一個筆記時要先找是哪個 database 這件事情後，那下一步就是解決 database
      過多的問題。
    </p>
    <h2 id="zheng3-li3-database"><a href="#zheng3-li3-database">整理 database</a></h2>
    <h3 id="wen2-zhang1-shou1-ji2"><a href="#wen2-zhang1-shou1-ji2">文章收集</a></h3>
    <p>
    ...
    <p>
      然後我將不管是個人生活相關或者技術之類的事情都放在同一個 database
    ...
    <p>
      但也因為是簡單的小程式所以我就沒有做一些 ranking
      的機制，就真的只是隨機抽出三篇文章而不會根據他已經被抽出幾次或者哪些主題太常被抽出來而降低優先之類的設計。
    </p>
    <p>原本這篇文章大概就到這裡結束只是後來在機緣巧合下得知了 heptabase 這個軟體，試用之後驚為天人就直接刷下去了。</p>
    <blockquote>
      <p>可惜知道的太晚，那時早鳥優惠剛過沒多久。</p>
    </blockquote>
    <h2 id="yu4-jian4-heptabase"><a href="#yu4-jian4-heptabase">遇見 heptabase</a></h2>
    <p>
      簡單介紹一下 heptabase
      ，最主要的特點是將各種筆記或者該說是知識、想法變成「卡片」然後在各個「白板」中組合出一個新的結構。
    </p>
    <blockquote>
      <p>
        可以看一下他們創辦人對於這三個軟體的比較 <a
          href="https://alanchan1209.medium.com/heptabase-vs-notion-vs-obsidian-3d1740c7745#6687"
          rel="external">
    ...
    <p>
      這跟使用 notion 時的概念完全不一樣，因為通常我是先想「我該寫在哪裡」之後在開始寫筆記。而 heptabase
      可以讓我「先寫就對了」之後再看怎麼整理。
    </p>
    <p>
      現在我都會習慣先把東西都寫在日記裡然後累積到一定程度再去把這些文字轉換成卡片，這件事情在 heptabase
      非常容易做到，我只要打完字然後拖曳過去白板就會是一張卡片了。
    </p>
    <ImgZoom src="/2023-10-24/heptabase1.png" alt="heptabase" class="h-full object-cover" />

    <p>heptabase 中「白板」就是我們在閱讀筆記以及整理知識最主要的地方</p>
    ...
    <p>經過這陣子的摸索我得出一個結論</p>
    <p><strong>不要妄想一個軟體可以做到所有事情</strong></p>
    <p>
      我目前就是變成 heptabase 為主，notion 為輔的筆記系統，因為他們各有強項誰也無法完全取代誰，hetpabase
      適合撰寫筆記、整理想法，notion 適合存放文獻、收集資料。
    </p>
    <p>
      所以目前我大部分有關「寫」這件事情都會在 heptabase 上處理，除了寫 blog 以外，畢竟 blog
      文章其實也算一種資料的群體所以在 notion 上撰寫我就可以簡單的分類、追蹤進度等等，只是我現在在寫 blog
      的同時另外一個螢幕就會是利用 hetpabase 來查閱我這次文章相關的知識，而不用像以前一樣要在 notion 中大海撈針。
    </p>
    <p>其餘不管是收集網路文章、旅遊景點、購物清單等等這種適合「用表格呈現」的行為我就還是會在 notion 裡處理。</p>
    <h1 id="jie2-yu3"><a href="#jie2-yu3">結語</a></h1>
    <p>
      我非常認同 heptabase
      團隊對於這個軟體的看法「比起一個筆記軟體，它更像一個學習軟體」，只是因為大部分時候我是透過筆記學習所以並沒有特別在乎兩者的差異，但用了
      heptabase
      之後我在複習一個知識時變得更加輕鬆，因為我可以很簡單地就可以讓一群相同或者對比的資料在同一個畫面上呈現，我覺得這點其他筆記軟體不是做不到但
      heptabase 的確把這些過程的阻力降到非常非常小了。
    </p>
    <p>
      總而言之我非常推薦這個軟體，除了本身好用以外 heptabase 非常認真在經營使用者社群。他們的 cs
      (其實我不確定是不是這個職稱，但總之就是負責客戶關係的成員)，前陣子開始提供預約諮詢的服務來討論有關 hetpabase
      的各種事情，後來我也有與他討論一些關於如何統整知識以及 heptabase 哪些功能可能可以達成我的需求。
    </p>
    <p>
      經過這次討論我覺得對於使用 heptabase 十分有幫助，畢竟 heptabase
      的核心概念上跟過去習慣使用的筆記軟體都不太一樣。試著改用他們建議的使用方法後使用體驗好非常多。
    </p>
    <p>在「回到Axton」這個 yt 頻道聽過這麼一段話原文有點忘了，大意大概是：</p>
    <p><strong>「最享受的浪費時間方式，就是研究各種提升效率的方法」</strong></p>
    <p>只能說，確實，其實現在我想做的事情用 notion 也許可以做到，但就是沒有那麼舒服而已。</p>
  </article>
</Post>
